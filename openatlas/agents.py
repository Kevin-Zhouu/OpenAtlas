...
READING NAVIGATION: Content owns the page. Do not duplicate OpenAtlas app branding/toolbars, add edition badges, a large permanent left sidebar, or a second progress dashboard. If navigation is helpful, default to one small rounded-rectangle Contents button at the left edge (roughly 44px high); activate by click/tap or keyboard to reveal the chapter list in a compact panel. Close on Escape, outside click and chapter selection; return focus appropriately. It must not cover reading text, and it must stay compact on phones. A short lesson may use only a small inline outline. Hide optional control panels until needed. Test the first viewport and a middle section: can a learner immediately tell what to read and what one interaction to try?
